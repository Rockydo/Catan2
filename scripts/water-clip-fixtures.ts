...
for (const river of [true, false])
  for (let mask = 0; mask < 64; mask++) {
    for (let basin = 0; basin < 64; basin++) {
      if ((!river && basin) || (basin & mask) !== basin) continue;
      const connections = { river, channel: mask, shore: mask, basin };
      const tile = {
      ...
      cases.push({
        ...
        mask,
        basin,
        path: river ? riverGeometry(mask, basin).water : waterSurfacePath(mask),
        svg,
      });
    }
  }
