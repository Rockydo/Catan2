...
Before every dice roll, compare each surviving human faction’s unrounded power with the total power of all surviving factions. If one human holds more than 40%, every surviving AI faction receives 1 Gold in each of its settlements and cities on that roll. This rises to 2 at 45%, 3 at 50%, 4 at 55%, and one more for each further five percentage points. Exactly 40% grants nothing.
Each AI receives its own full payment. A city counts as one site, regardless of level. The bonus is paid on human and AI rolls, regardless of dice result, climate, season, blockade or siege, and is stored in the receiving town. It produces no Gold bars. Harvest reports include the delivered Gold and identify the support share; the faction view shows the current rate.
Only human power can activate this rule. Multiple humans do not pool their power, and humans never receive the bonus. Use the largest individual human share. Recalculate on every roll: the bonus decreases with that share and stops immediately at 40% or below, independently of the coalition’s 20% release threshold. The power formula uses ordinary production, excluding support, so the bonus cannot change its own threshold. Loading an existing save enables the rule for the next roll without retroactive payments.
