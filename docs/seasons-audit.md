...
A season lasts two complete rounds: early and late. A year lasts eight rounds. Spring, Summer, Autumn and Winter have the same number of faction turns for a stable set of survivors. Each season is a harvest window: every matching dice roll pays. There is no once-per-year payout cap and no compensation when a harvest window receives no matching roll.

For each current climate-adjusted resource baseline Y, its four seasonal payouts sum to 4Y. Across the eight-round year the scheduled total is 8Y. Expected annual land production is therefore: 8 × living factions × dice probability × Y; expected production per round stays unchanged. Actual frozen water can suppress scheduled marine harvests. This must hold independently for every component of mixed terrain, including whale Oil, seal Oil, Oasis food and wood, and Woods choices. A wheat tile on 7 can miss an entire five-faction harvest season with probability (5/6)^10, about 16%. A tile on 2 misses with probability (35/36)^10, about 75%.
...
## Half-season sea-freezing contract

`calendar.roundsPerSeason: 2` and `iceModel: 2` enable the current calendar and weather. `ICE_TRANSITIONS` stores conditional freeze/melt probabilities for all eight halves. `iceOdds` handles Arctic pack ice and permanent Glacial pack. `iceWeather` records the resolved round, season and half on every sea tile; `surface` records the result. Resolve once at each full-round boundary, independently per tile. Repeated synchronization and reloads are idempotent. A seed/hex/round hash varies weather between years without advancing dice, research, climate or other random streams.

The original `freezeRoll` and `SHOULDER_ICE_CHANCE` now preserve the existing local harvest calendar only. They must not force the physical surface in model 2. Early and late share the same scheduled per-roll yields and seasonal artwork. Actual frozen water suppresses marine harvest, including both Whale goods and their processed outputs. No weather loss is transferred or compensated. `iceRisk` computes public probabilities from the resolved surface and transition table, without reading future weather draws. The AI forecasts these probabilities and evacuates likely hazards.

Version 13 migration preserves the current main season, year, ice surfaces, units, inventories and random streams. The current round becomes the early half; the next full-round boundary checks weather normally. Legacy saves without a calendar still activate at the next full round. Original harvest grace may remain until the main season changes, but physical weather can change at either half. Validate weather metadata and permanent pack ice, and reject invalid saved state.

## Surface-transition safety

Use one deterministic global pass at every half-season boundary. Never kill units automatically, place them on invalid terrain, or silently let opposing land/naval forces share a tile. Any protected occupancy or rescue exception must be explicit in the UI and save invariants. Be careful with ships carrying troops, fleets on isolated lakes, ice tiles with armies and no reachable shore, retreat reservations, and former allies sharing a tile.
...
5. Exactly one half-season transition per whole round and one main season per two rounds, dead factions skipped correctly, all-player elimination/victory handling, setup resource baseline.
6. Exact conditional freeze/thaw probabilities, changing weather between years, stable results on reload, and distinct Arctic seasonal pack and Glacial permanent ice exceptions. Exercise freeze/thaw with land units, naval units, loaded carriers, enemy stacks, no shore, and build legality.
7. Version-1–8 Spring activation; versions 9–12 migration preserving current year, season, ice and occupied water; version-13 roundtrips in every half-season without data loss or rerolling; quarter-power fleets with normal land escorts in direct attacks and bombardment; no icebound retreat.
