...
Decorative map tokens, town miniatures and force badges are prepared once as shared, lossless images at the maximum camera zoom and display density. Dice, resource rows and their backings are shared separately to avoid storing every combination. Selection, counts, hit targets and accessible labels remain live. Unused prepared images are released from a bounded cache. Image preparation failures retain the original vectors.

Maps with at least 800 tiles use a WebGL 2 terrain layer when available. The camera draws the existing artwork directly at the current viewport and display density; zooming does not regenerate the terrain or production labels. Texture storage has a 512 MiB limit, including mipmaps. Mode changes release the textures, and artwork changes reuse unchanged images where possible. Unsupported graphics, failed preparation or context loss keep the original SVG map available. The interactive map remains SVG in both cases.

The report records the actual graphics renderer and acceleration status. The default headless mode uses software rendering on this machine. Set `GRAPHICS=hardware` to request GPU acceleration; the diagnostic fails if hardware compositing is unavailable. Compare results within the same graphics mode. Use `WIDTH`, `HEIGHT` and `DPR` to match the display being tested. Reports include these settings and counts of prepared map sprites before and after navigation. `WAIT_SPRITES=1` waits for sprite preparation before sending camera input and reports that extra wait separately; omit it to include navigation during startup. This wait does not establish that every terrain artwork download has finished. For maps using GPU terrain, `WAIT_GPU=1` also waits for that layer and reports its allocated texture bytes. Omit these options to exercise navigation during preparation. Use `PROFILE_CAMERA=1` to write a CPU profile and `TRACE_CAMERA=1` to write a browser timeline beside the report.
