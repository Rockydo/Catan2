...
`public/assets/seasons/` contains painterly seasonal landscapes for every generated climate and terrain combination. The four variants show crop growth and harvest, shearing and livestock, snow, ice, thaw, and warm-climate wet/dry seasons. New barley, rye, millet, maize, cattle, goat and reindeer terrain also has a separate base illustration for references outside the calendar.

All seasonal paintings were generated with the built-in image generation tool. Four-season atlas cells were extracted and encoded as local WebP files. This packaging does not recolor or repaint the generated artwork. `public/assets/seasons/prompts.json` records the prompts, layout and source identifiers. Solid winter ice is intentionally reused across the northern water resources: the map's resource symbol and dice label identify what lies beneath it. No external service is contacted while playing.

Arctic autumn land art uses six revised `public/assets/seasons/arctic-*-autumn-v2.webp` images: Snow plain, Seal grounds, Reindeer range, Iron, Stone and Gold. Snow covers most of the ground while patches of autumn vegetation remain visible. Spring keeps its existing snow and Winter retains its deeper cover. These individual edits use the built-in image generation tool; `public/assets/seasons/arctic-autumn-prompts.json` records their exact prompts, references and output paths. Earlier images remain available for already-open campaigns.

`public/assets/seasons/manifest.json` and its identical source mirror, `src/ui/season-art-manifest.json`, map each climate/terrain/season to its asset. The board loads only textures used by the campaign and selected season. Seasonal images share a hexagonal clip instead of separate SVG pattern fills, keeping map zoom fast. Seasonal previews change illustrations only. Terrain movement and production always use the actual campaign season.

The Meat resource uses an original scalable steak pictogram in `src/ui/ResourceIcon.tsx`. Calendar symbols use the existing Lucide icon set. Production quantities, names and dice numbers remain separate vector labels so every season is readable at map scale. Crop-stage reviews use the same central hex crop as the game; automated coverage checks require all four seasonal assets for every terrain that the generator can create.
