...
## Dense-map renderer investigation: 2026-09-23

- No renderer change from this investigation was deployed. The verified PNG-sprite renderer from `62dd1f6` remains in the playable build. Terrain-canvas prototypes, extra clipping and compositor-layer changes were measured and removed when they failed to improve the complete navigation workload while preserving image quality.
- On the 2,000-tile fixture, a canvas prototype reduced some zoom measurements but made overview dragging slower and initially exposed a viewport-edge coverage problem. Retaining a painted border restored drag p95 to about 16.8 ms. High-quality image filtering then raised zoom p95 to 133–167 ms. Precomputed smaller terrain images avoided that filtering cost, but the complete candidate still measured 67–100 ms zoom p95 and failed a zoomed artwork comparison. These results do not support replacing the existing renderer. Prototype code and reports are retained under ignored `test-artifacts/`, including `performance-goal/rejected-canvas/` and the `camera-canvas-*` reports.
- An additional accelerated Chromium run uses the unchanged Round 32 export at 1920×1080 CSS pixels and DPR 2. After waiting another 1,060 ms for outstanding sprite preparation, all eight navigation segments measure 16.7–16.8 ms frame-time p95. One rapid zoom-out segment has a 66.6 ms maximum; the other zoom maxima are at most 33.4 ms and all drag maxima are 16.8 ms. The complete save is unchanged and no browser errors occur. This establishes high-DPI behavior on that export, not a bound for larger campaigns. Evidence: `test-artifacts/camera-ready-dpr2-round32.{json,log,png}`.
- The durable camera diagnostic now records viewport size, display density and per-kind sprite readiness. Optional `WAIT_SPRITES=1` isolates steady navigation and reports the excluded preparation wait; the default still includes startup overlap. The load diagnostic's optional `TRACK_SPRITES=1` reports Continue-to-sprite readiness separately from two rendered frames. Neither measure claims that every terrain image has finished downloading.
- The new load measurement passes three exact Round 32 reloads: Continue-to-map takes 728/410/429 ms; prepared sprites take 2,061/1,715/1,748 ms from that same click. Report: `test-artifacts/save-load-ready-diagnostic-round32.json`. Production build, TypeScript, formatting, whitespace and all 12 HTTP checks pass. The diagnostics run against disposable profiles and compare complete saves. No playing browser, live storage or exported campaign was modified. The performance goal remains active for denser growing maps.

## Hardware-accelerated map navigation: 2026-09-23

- Hardware profiling reproduced the reported lag even when camera JavaScript was fast. Chromium repeatedly rasterized SVG images containing resource icons, dice tokens and miniature details. The renderer now prepares the identical artwork as shared lossless PNG sprites. Preparation uses the maximum map zoom, window bounds and device pixel ratio. Camera movement reuses these images without regenerating them. Active sprites remain shared; the cache retains only 64 unused images and releases their blob URLs on eviction. Failed or stalled preparation keeps the original playable vectors.
- On the Round 32 export (540 tiles, 322 towns, 14,695 units), accelerated Chromium zoom frame-time p95 falls from 133–150 ms to 16.7–16.8 ms. All four drag segments also measure 16.7–16.8 ms p95. The first zoom sequence still includes four startup frames above 33.5 ms, with a 150 ms maximum while initial artwork preparation settles; subsequent measured segments have no such frames. No JavaScript long tasks or browser errors occur in that run. These are local RTX 5090/OpenGL measurements, not universal frame-rate guarantees.
- A denser 2,000-tile, 1,000-town fixture improves from 417–517 ms zoom p95 to 67–83 ms. Close-up dragging improves from 33.3 ms p95, with 217–250 ms peaks, to 16.8–33.3 ms p95 and 50 ms peaks. This larger scene still needs further rendering work; the ongoing performance goal remains active. Diagnostic-only hiding of texture layers, foreground or filters was used to isolate costs and is not shipped.
- Complete saved campaigns are unchanged after camera input. Round 32 refresh/open comparisons also preserve the exact campaign: cold Continue-to-map samples are 724 ms before and 791 ms after; warm samples are 458/469 ms before and 424/422 ms after. This measures two rendered frames, not completion of all image preparation, and does not claim a loading speedup.
- The camera diagnostic now measures real dragging at both overview and close-up scales. Hardware reports use `test-artifacts/camera-latency-*`; load reports use `save-load-camera-*`. Testing uses exported copies in disposable profiles, never the playing browser.
- All 1,833 unit tests in 140 files pass. All 93 affected browser scenarios pass across Chromium, Firefox and mobile Chromium after correcting tests that sent keyboard input before the initially hidden map appeared. The final targeted rerun passes all 21 checks. Coverage includes pixel comparisons against the source artwork, maximum-zoom resolution on high-DPI displays, unchanged image URLs during camera input, shared army art and live counts, selection, walls, city upgrades, guilds, failed/stalled decoders, culling, seasonal previews, both languages and save preservation. Firefox alpha round-trips are allowed two of 255 visible channel levels; Chromium comparisons match within that same bound. TypeScript, build and changed-code formatting checks pass.
- The deployed build passes all five focused Chromium checks and all 12 HTTP checks. The pixel test reads captured image MIME types instead of fetching blob URLs, preserving the server’s strict connection policy. Existing hashed assets remain available to open games.
